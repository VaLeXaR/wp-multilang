	...
	public function add_language_fields( $widget, $return, $instance ) {
		...
		<p>
			...
			<?php foreach ( $languages as $code => $language ) { ?>
				<label><input type="checkbox" name="<?php echo esc_attr_e( $widget->get_field_name('languages') ); ?>[<?php echo esc_attr( $i ) ; ?>]" id="<?php echo esc_attr( $widget->get_field_id('languages') . '-' . $code ); ?>" value="<?php echo esc_attr( $code ); ?>"<?php checked( in_array( $code, $instance['languages'] ) ); ?>><?php echo esc_attr( $language['name'] ); ?></label><br>
			<?php $i++; } ?>
		</p>
		<?php
	}
