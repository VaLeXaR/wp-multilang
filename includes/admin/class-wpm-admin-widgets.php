	...
	public function add_language_fields( $widget, $return, $instance ) {
		...
		<p>
			...
			<?php foreach ( $languages as $lang => $language ) { ?>
				<label><input type="checkbox" name="<?php esc_attr_e( $widget->get_field_name('languages') ); ?>[<?php esc_attr_e( $i ); ?>]" id="<?php echo $widget->get_field_id('languages') . '-' . $lang; ?>" value="<?php esc_attr_e( $lang ); ?>"<?php if ( in_array( $lang, $instance['languages'] ) ) { ?> checked="checked"<?php } ?>><?php echo $language['name']; ?></label><br>
			<?php $i++; } ?>
		</p>
		<?php
	}
