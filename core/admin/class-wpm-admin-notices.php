...
use WPM\Core\WPM_Background_Updater;
...
class WPM_Admin_Notices {
	...
	public static function init() {
		...
		add_action( 'shutdown', array( __CLASS__, 'plugin_notices' ) );
		add_action( 'admin_print_styles', array( __CLASS__, 'add_notices' ) );
	}

	/**
	 * Plugin notices to DB
	 */
	...
	public static function hide_notices() {
		if ( isset( $_GET['wpm-hide-notice'] ) && isset( $_GET['_wpm_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_wpm_notice_nonce'], 'wpm_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'wpm' ) );
			}
